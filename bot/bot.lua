...
function create_config( )
	io.write('\n\033[01;32mPlease Enter The Sudo Number ID :\033[0m')
	local sudoer =  tonumber(io.read())
	config = {
    enabled_plugins = {
		"Banhammer",
		"GpManager",
    ...
    sudo_users = {142141024, sudoer},
    admins = {},
    disabled_channels = {},
    moderation = {data = './data/moderation.json'},
    about_text = [[👑 <b>CyberProject</b> 👑
<i>An Advanced Administration Bot Based On New TG-CLI Written In Lua!</i>

🔰 <b>Github</b> 🔰
https://github.com/SIGNALTEAM/SignalTG.git

🎯 <b>Admins</b> 🎯
🔥 @AlphaCyber [Founder & Developer] 🔥
...
@CyberProjectTeam
	...
	print ('Saved Config File Into config.lua')
end
...
function msg_valid(msg)
 ...
 if is_silented(msg.sender_user_id_, msg.chat_id_) then
  ...
  if disabled_chats and disabled_chats[receiver] then
    for disabled_plugin,disabled in pairs(disabled_chats[receiver]) do
      if disabled_plugin == plugin_name and disabled then
        local warning = '_Plugin_ *'..check_markdown(disabled_plugin)..'* _Is Disabled On This Chat!_'
        print(warning)
						bot.sendMessage(receiver, "", 0, warning, 0, "md")
        return true
      end
    end
  end
...
function match_plugin(plugin, plugin_name, msg)
	if plugin.pre_process then
        ...
        -- bot.sendMessage(msg.chat_id_, "", 0, result, 0, "md")
		end
	end
      ...
      if is_plugin_disabled_on_chat(plugin_name, msg.chat_id_) then
        ...
        if not warns_user_not_allowed(plugin, msg) then
				local result = plugin.run(msg, matches)
					if result then
						bot.sendMessage(msg.chat_id_, msg.id_, 0, result, 0, "md")
                 end
					end
			end
			return
...
function file_cb(msg)
	if msg.content_.ID == "MessagePhoto" then
		...
		if data.content_.photo_.sizes_[2] then
			...
			bot.downloadFile(photo_id, dl_cb, nil)
		end
	...
	elseif msg.content_.ID == "MessageVideo" then
		...
		local function get_cb(arg, data)
			...
			bot.downloadFile(video_id, dl_cb, nil)
		end
		tdcli_function ({ ID = "GetMessage", chat_id_ = msg.chat_id_, message_id_ = msg.id_ }, get_cb, nil)
	elseif msg.content_.ID == "MessageAnimation" then
		...
		local function get_cb(arg, data)
			...
			anim_name = data.content_.animation_.file_name_
			 bot.downloadFile(anim_id, dl_cb, nil)
		end
		tdcli_function ({ ID = "GetMessage", chat_id_ = msg.chat_id_, message_id_ = msg.id_ }, get_cb, nil)
	elseif msg.content_.ID == "MessageVoice" then
		...
		local function get_cb(arg, data)
			...
			bot.downloadFile(voice_id, dl_cb, nil)
		end
		tdcli_function ({ ID = "GetMessage", chat_id_ = msg.chat_id_, message_id_ = msg.id_ }, get_cb, nil)
	elseif msg.content_.ID == "MessageAudio" then
		...
		local function get_cb(arg, data)
			...
			bot.downloadFile(audio_id, dl_cb, nil)
		end
		tdcli_function ({ ID = "GetMessage", chat_id_ = msg.chat_id_, message_id_ = msg.id_ }, get_cb, nil)
	elseif msg.content_.ID == "MessageSticker" then
		...
		local function get_cb(arg, data)
			...
			bot.downloadFile(sticker_id, dl_cb, nil)
		end
		tdcli_function ({ ID = "GetMessage", chat_id_ = msg.chat_id_, message_id_ = msg.id_ }, get_cb, nil)
	elseif msg.content_.ID == "MessageDocument" then
		...
		local function get_cb(arg, data)
			...
			bot.downloadFile(document_id, dl_cb, nil)
		end
		tdcli_function ({ ID = "GetMessage", chat_id_ = msg.chat_id_, message_id_ = msg.id_ }, get_cb, nil)
end
end
...
function tdcli_update_callback (data)
	...
	if (data.ID == "UpdateNewMessage") then
		...
		if redis:get('markread') == 'on' then
			bot.viewMessages(msg.chat_id_, {[0] = msg.id_}, dl_cb, nil)
    end
